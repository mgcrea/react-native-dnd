...
<Properties>
  ...
  <Property name="activationDelay" type="number" optional defaultValue={`0`}>
    An optional number representing the duration, in milliseconds, of the LongPress gesture before Pan is allowed to activate.
    If the finger is moved during that period, the gesture will fail. Default is `0`.
  </Property>
  <Property name="minDistance" type="number" optional defaultValue={`0`}>
    An optional number representing minimum distance the finger (or multiple finger) need to travel before the gesture activates.
    Expressed in points.
  </Property>
